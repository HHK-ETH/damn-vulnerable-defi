...
describe('[Challenge] Puppet', function () {
  ...
  it('Execution', async function () {
    ...
    permit = ethers.utils.splitSignature(
      await player._signTypedData(
        {
        ...
        },
        {
          ...
          spender: ethers.utils.getContractAddress({
            from: player.address,
    ...
    const factory = await ethers.getContractFactory('AttackPuppet', player);
    const attackContract = await factory
      .connect(player)
      .deploy(
        player.address,
        token.address,
        ...
        permit.r,
        permit.s,
        { value: parseUnits('24'), gasLimit: 1e7 }
      );
  });
